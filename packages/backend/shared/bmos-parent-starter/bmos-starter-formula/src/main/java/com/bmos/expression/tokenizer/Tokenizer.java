...
import com.bmos.expression.operator.Operators;
...
//                    throw new IllegalArgumentException("Unable to parse char '" + ch + "' (Code:" + (int) ch + ") at [" + pos + "]");
                    throw new BmosException(BaseResponseCode.UNABLE_PARSE_CHAR, String.valueOf(ch));
...
//        throw new IllegalArgumentException("Unable to parse char '" + ch + "' (Code:" + (int) ch + ") at [" + pos + "]");
    ...
    private Token parseFunctionOrVariable() {
        ...
        if (lastValidToken == null) {
            lastValidLen = len;
            lastValidToken = new ParamToken(new String(expression, offset, len - 1));
