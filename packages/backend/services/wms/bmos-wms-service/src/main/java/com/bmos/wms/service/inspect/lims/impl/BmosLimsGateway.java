package com.bmos.wms.service.inspect.lims.impl;
...
import com.bmos.wms.service.inspect.controller.vo.InspectConfigDataVO;
...
import com.bmos.wms.service.inspect.lims.LimsType;
 ...
 *
 * <p>所有出向 DTO 的 sourceSystem 显式写为 "WMS"，LIMS 端据此回调到 WmsInspectCallbackClient。
 */
@Component
public class BmosLimsGateway implements LimsInspectGateway {

    private static final String SOURCE_WMS = "WMS";
    ...
    private InspectConfigDetailVO toVO(MesDocumentConfigFeignVO cfg) {
    ...
    @Override
    public List<InspectSchemeVO> querySchemes(Long platformMaterialId) {
        List<MesSchemeFeignVO> list = FeignUtils.handleRequest(
                data -> mesInspectFeign.querySchemes(data), platformMaterialId).getData();
    ...
    public String initiate(InitiateInspectContext ctx) {
        ...
        dto.setSourceSystem(SOURCE_WMS);
    ...
    /** 把 InspectInfo 列表拆成 fields / fieldNames / fieldRequired 三个 map（与 MES 同实现）。 */
    private void fillFieldMaps(List<InspectInfo> infos,
                               Consumer<Map<String, String>> fieldsSetter,
                               Consumer<Map<String, String>> namesSetter,
                               Consumer<Map<String, Boolean>> requiredSetter) {
