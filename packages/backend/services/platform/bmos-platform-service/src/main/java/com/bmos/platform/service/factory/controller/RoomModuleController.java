 ...
 */
@RestController
@RequestMapping("/factory/room/module")
@Validated
@Api(tags = "房间模型树相关接口")
public class RoomModuleController {
    ...
    public ResponseInfo<Void> saveModule(@RequestBody ModuleSaveDTO dto){
        factoryModuleService.saveModule(dto, FactoryModuleEnum.ROOM);
    ...
    @ApiOperation("获取设备房间模型树(不包含房间信息)")
    public ResponseInfo<List<StationModuleTreeNodeVO>> getModuleTree(){
        return ResponseInfo.success(factoryModuleService.getModuleTree(FactoryModuleEnum.ROOM));
    }

    @GetMapping("/info/{id}")
    @ApiOperation("获取房间模型基础信息")
