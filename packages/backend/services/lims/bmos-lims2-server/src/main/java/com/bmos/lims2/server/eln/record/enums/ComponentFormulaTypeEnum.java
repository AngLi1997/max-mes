...
import com.bmos.lims2.common.model.execute.ExecuteFormDataTimeExtInfo;
...
import com.bmos.lims2.server.eln.record.entity.formula.DateCalculateConfig;
import com.bmos.lims2.server.eln.record.entity.formula.NumericalJudgmentConfig;
    ...
    TIME_DIFF_FORMULA("3", "时间差公式", null, false, (batchRecordComponent, keys, values, function) -> {
        ...
        List<ExecuteFormDataBaseExtInfo> infos = collect.stream().map(item -> {
            ...
            if (extInfo == null || StrUtil.isEmpty(extInfo.getTimeStamp())) {
                throw new BmosException(LimsResponseCode.TIME_DIFF_FORMULA_PARAM_HAS_NO_TIMESTAMP);
            }
            return extInfo;
        }).collect(Collectors.toList());
