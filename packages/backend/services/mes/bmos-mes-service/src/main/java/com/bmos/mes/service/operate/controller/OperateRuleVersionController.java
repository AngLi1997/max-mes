...
public class OperateRuleVersionController {
    ...
    @GetMapping("/update/effect")
    @ApiOperation(value = "立即生效")
    @ApiParam(name = "id",value = "版本id",required = true)
    public ResponseInfo<Boolean> updateEffect(@NotNull @Valid Long id){
        return ResponseInfo.success(versionService.updateEffect(id));
    }
    ...
    @ApiParam(name = "stepModelId",value = "工步模型id",required = true)
    public ResponseInfo<List<OperateRule>> getAppDetail(@NotNull @Valid Long stepModelId){
        return ResponseInfo.success(versionService.getAppDetail(stepModelId));
    }
}
