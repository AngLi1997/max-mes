...
@Data
@Slf4j
public class StepCompleteCompleteType extends ConditionChangeType {
    private Long stepId;

    public StepCompleteCompleteType(Long planId, Long stepId) {
        super(planId);
        this.stepId = stepId;
    }
    ...
    public ConditionTypeEnum getConditionType() {
        return ConditionTypeEnum.STEP_NODE_COMPLETE;
    }
    ...
    public void innerCalculateConditionChange(List<ProcedureConditionInstance> changeConditionInstances,
        ...
        changeConditionInstances.forEach(item -> {
            ...
            if (conditionDetailVO.getStepId().equals(stepId)) {
                log.info("计划id：【{}】步骤模型id:【{}】条件【{}】配置的步骤【{}】完成条件完成", this.planId, item.getProcedureStepModelId(),
                        item.getName(), conditionDetailVO.getStepName());
