...
public class MinioFileClient {
    ...
    public String getBucketName(MinioBucket bucket) {
        switch (bucket) {
            case ARCHIVE_BUCKET:
                return minioProperties.getBuckets().getArchive();
            case RECORD_BUCKET:
                return minioProperties.getBuckets().getRecord();
            case OPERATE_RULE_SOP:
                return minioProperties.getBuckets().getSop();
            case BMOS_PRODUCT:
                return minioProperties.getBuckets().getProduct();
            case BMOS_BATCH_TEMPLATE:
                return minioProperties.getBuckets().getBatchRecordTemplate();
            case BMOS_LOT_RELEASE:
                return minioProperties.getBuckets().getLotRelease();
        }
    ...
    public void download(MinioBucket bucket, String path, HttpServletResponse response) throws Exception {
        try (OutputStream outputStream = response.getOutputStream()) {
            ...
            e.printStackTrace();
            throw new BmosException(MesResponseCode.FILE_DOWNLOAD_FAILED);
        }
    }
