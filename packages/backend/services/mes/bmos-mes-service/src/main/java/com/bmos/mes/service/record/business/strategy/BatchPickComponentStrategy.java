 ...
 * 按批次量领料
 */
@Service(value = "BATCH_QUANTITY_PICK")
public class BatchPickComponentStrategy implements BusinessComponentStrategy {

    @Autowired
    private BatchPickMaterialBatchComponentStrategy materialBatchComponentStrategy;

    @Autowired
    private BatchPickMaterialSumComponentStrategy sumComponentStrategy;
    ...
    public void handleBusinessComponent(List<ExecuteFormData> results, ComponentListVO component,
        ...
        List<ComponentListVO> pickBatch = childMap.get(BusinessComponentTypeEnum.BATCH_QUANTITY_PICK_BATCH.getValue());
        if (CollUtil.isNotEmpty(pickBatch)) {
            for (int i = 0; i < pickBatch.size(); i++) {
                ComponentListVO child = pickBatch.get(i);
                materialBatchComponentStrategy.handleBusinessComponent(results, child, info, configMap, i);
            }
        }
        List<ComponentListVO> summary = childMap.get(BusinessComponentTypeEnum.BATCH_QUANTITY_PICK_SUMMARY.getValue());
