 ...
 * 配料计划
 */
@Service(value = "INGREDIENTS_PLAN")
public class IngredientPlanComponentStrategy implements BusinessComponentStrategy {

    @Autowired
    private IngredientPlanMaterialComponentStrategy materialComponentStrategy;
    ...
    public void handleBusinessComponent(List<ExecuteFormData> results, ComponentListVO component,
        ...
        List<ComponentListVO> collect = children.stream().filter(c -> ObjectUtil.isNotNull(configMap.get(c.getId()))).collect(Collectors.toList());
