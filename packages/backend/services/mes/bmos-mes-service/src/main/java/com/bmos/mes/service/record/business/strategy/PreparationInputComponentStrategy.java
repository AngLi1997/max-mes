 ...
 * 配液投入组件
 */
@Component("LIQUID_PREPARATION_INPUT")
public class PreparationInputComponentStrategy  implements BusinessComponentStrategy {

    @Autowired
    PreparationInputDetailComponentStrategy detailComponentStrategy;

    @Autowired
    PreparationInputSummaryComponentStrategy summaryComponentStrategy;
    ...
    public void handleBusinessComponent(List<ExecuteFormData> results, ComponentListVO component,
        ...
        for (String componentType : componentTypeMap.keySet()) {
            ...
            if (BusinessComponentTypeEnum.LIQUID_PREPARATION_INPUT_DETAIL.getValue().equals(componentType)){
