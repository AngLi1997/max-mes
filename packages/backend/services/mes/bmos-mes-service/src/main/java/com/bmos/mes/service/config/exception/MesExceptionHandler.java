...
import static com.bmos.mes.common.exception.MesResponseCode.ORCHESTRATOR_ENGINE_ERROR;
...
public class MesExceptionHandler {
    ...
    public ResponseInfo<?> authorizationException(InfiniteEngineException ex) {
        if (StrUtil.equals(ex.getMessage(),ERROR)){
            return ResponseInfo.failure(ORCHESTRATOR_ENGINE_ERROR, ex.getMessage(), ex.getProps());
        }
