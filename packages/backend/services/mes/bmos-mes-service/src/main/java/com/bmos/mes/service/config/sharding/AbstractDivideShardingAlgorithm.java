...
public abstract class AbstractDivideShardingAlgorithm {
    ...
    protected List<String> getRuleRange(LocalDateTime lowerEndpoint, LocalDateTime upperEndpoint) {
        ...
        // lowerDate那一年需要的规则
        result.addAll(mergeRule(Lists.newArrayList(Integer.toString(lowerYear % YEAR_RULE)), getRuleMonthRange(lowerEndpoint, TimeUtil.endOfYear(lowerEndpoint))));
        // upperDate那一年需要的规则
        result.addAll(mergeRule(Lists.newArrayList(Integer.toString(upperYear % YEAR_RULE)), getRuleMonthRange(TimeUtil.beginOfYear(upperEndpoint), upperEndpoint)));
        // 获取中间年的规则
