...
# 从配置文件读取参数（build_config.ini 与本脚本同目录）
_config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'build_config.ini')
_config = configparser.ConfigParser()
_config.read(_config_path, encoding='utf-8')
if not _config.has_section('settings'):
    raise FileNotFoundError(f"配置文件未找到或格式错误：{_config_path}")
_s = _config['settings']

release_type = _s.get('release_type')
project_type = _s.get('project_type')
source_release = _s.get('source_release')
target_release = _s.get('target_release')
root_directory = _s.get('root_directory')
workspace_ip = _s.get('workspace_ip')
workspace_user = _s.get('workspace_user')
workspace_pwd = _s.get('workspace_pwd')
system_type = _s.get('system_type')
...
with open('0-initClear.sql', 'r', encoding='utf-8') as initClear_file:
    initClear_sql = initClear_file.read()
with open(merge_sql, 'w', encoding='utf-8') as outfile:
    outfile.write(initClear_sql)
merge_source_sql = os.path.join(source_release_directory, 'target/init.sql')
with open(merge_source_sql, 'r', encoding='utf-8') as source_init:
    init_sql = source_init.read()
with open(merge_sql,'a', encoding='utf-8') as  outfile:
    outfile.write('\n')
    ...
    for key in roots.keys():
        ...
        if os.path.exists(target_upgrade_sql):
            with open(target_upgrade_sql, 'r', encoding='utf-8') as infile:
...
with open(upgrade_sql_path, 'w', encoding='utf-8') as outfile:
...
with open(reset_sql_path, 'w', encoding='utf-8') as outfile:
...
with open(init_sql_path, 'w', encoding='utf-8') as init_file:
    with open('0-initClear.sql', 'r', encoding='utf-8') as clear_script:
        init_file.write(clear_script.read())
    with open(reset_sql_path, 'r', encoding='utf-8') as reset_script:
