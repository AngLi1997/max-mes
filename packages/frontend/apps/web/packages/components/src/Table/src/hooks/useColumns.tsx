...
export const useColumns = ({ state, methods, props, tableAction }: UseTableColumnsContext): any => {
  ...
  // 监听getProps变化，根据 dataIndex 更新 useInnerColumns中的 title
  watchEffect(() => {
    const columns = cloneDeep(unref(getProps)!.columns);
    useInnerColumns.value = columns.map(item => {
      if (item.dataIndex && item.title) {
        const title = unref(getProps)?.[item.dataIndex as string]?.title;
        if (title) {
          item.title = title;
        }
      }
