...
import { ConfigProviderInjection } from '../../../ConfigProvider/src/types';

export const useVirtualized = ({ state }: { state: TableState }) => {
  const configProvider = inject<ConfigProviderInjection>('bmos-config-provider');

  const clsPrefixRef = configProvider?.clsPrefixRef.value;
  ...
  const parentNode = ref(document.querySelector(`.${clsPrefixRef || 'ant'}-table-body`) as HTMLDivElement);
  const contentNode = ref(document.querySelector(`.${clsPrefixRef || 'ant'}-table-body table`) as HTMLDivElement);
  ...
  const initVir = () => {
    parentNode.value = document.querySelector(
      `.${virtualizedClass.value} .${clsPrefixRef || 'ant'}-table-body`,
    ) as HTMLDivElement;
    contentNode.value = document.querySelector(
      `.${virtualizedClass.value} .${clsPrefixRef || 'ant'}-table-body table`,
    ) as HTMLDivElement;
