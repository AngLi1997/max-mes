<template>
  <div class="detail">
    ...
    <div class="batch-upload-topic">
      {{ t('本次导入成功标本数量') }}：
      <span class="suc">{{ `${numObj.successNum}${t('份')}` }}</span>
      ，{{ t('本次导入失败标本数量') }}：
      <span class="fail">{{ `${numObj.errorNum}${t('份')}` }}</span>
    </div>
    <div class="batch-upload-btn">
      <!-- <Button :disabled="numObj.errorNum" type="primary" @click="next">{{ t('下一步') }}</Button> -->
...
<script setup lang="tsx">
  ...
  const numObj = computed(() => {
    return {
      errorNum: props.info?.errorNum ?? 0,
  ...
  onMounted(() => {
    tableData.value = props.info?.errorData ?? [];
  });
</script>
