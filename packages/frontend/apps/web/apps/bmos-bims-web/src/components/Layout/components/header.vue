...
<script setup lang="ts">
  ...
  const queryWarp = () => {
    ...
    const tar = headerTabsREF.value?.$el?.querySelector('.bims-tabs-nav-wrap');
    if (!tar) return;
...
<style scoped lang="css">
  ...
  :deep(.header-tabs .bims-tabs-nav) {
    margin-bottom: 0;
  }

  :deep(.header-tabs .bims-tabs-tab) {
    border: none;
  }
  :deep(.header-tabs .bims-tabs-nav-operations) {
    display: none;
  }
</style>
