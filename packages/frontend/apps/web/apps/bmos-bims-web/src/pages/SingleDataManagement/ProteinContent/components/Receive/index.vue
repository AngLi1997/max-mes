...
<template>
  <BMModalForm
    ref="modalFormRef"
    v-model:open="open"
    :title="dialogType == 'publish' ? t('蛋白质含量发布') : t('蛋白质含量核对')"
...
<script setup lang="ts">
  ...
  import { publishProteinContent, checkProteinContent } from '@/services';
  ...
  const submit = async () => {
    try {
      ...
      if (dialogType.value === 'publish') {
        await publishProteinContent(params);
      } else {
        await checkProteinContent(params);
      }
