...
<script setup lang="ts">
  ...
  import { getIncomingMaterialDetail, createIncomingMaterial, updateIncomingMaterial } from '@/services';
  ...
  const openModal = async (row: any, type: 'create' | 'update') => {
    ...
    let formData = {};
    if (type === 'update') {
      const { data } = await getIncomingMaterialDetail(row.id);
      formData = {
        ...data,
  ...
  const request = async (formModal: any) => {
    try {
      if (dialogType.value === 'create') {
        return await createIncomingMaterial(formModal);
      } else {
        return await updateIncomingMaterial(formModal);
      }
