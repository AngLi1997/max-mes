...
<script setup lang="ts">
  ...
  import { batchPublishProteinContent, batchCheckProteinContent } from '@/services';
  ...
  const request = async (formModal: any) => {
    try {
      if (actionType.value == 'publish') {
        return await batchPublishProteinContent(formModal);
      } else {
        return await batchCheckProteinContent(formModal);
      }
    } catch (error) {
