  ...
  <BMPageComponent
    ...
    <template #tableHeaderTitle0>
      <div class="table-header">
        ...
        <Button v-hasAuth="180020008000002" type="primary" style="margin-right: 8px" @click="openBatch('publish')">
          {{ t('批量发布') }}
        </Button>
        <Button
          v-hasAuth="180020008000003"
          :disabled="disCheck()"
          style="margin-right: 8px"
          @click="openReceive(operationSelectedRows, 'check')">
          {{ t('核对') }}
        </Button>
        <Button v-hasAuth="180020008000004" type="primary" style="margin-right: 8px" @click="openBatch('check')">
          {{ t('批量核对') }}
        </Button>
      </div>
    </template>
    <template #tableHeaderToolbar0>
      <Button v-hasAuth="180020008000005" @click="openImport">
        {{ t('读取') }}
      </Button>
    </template>
  </BMPageComponent>
