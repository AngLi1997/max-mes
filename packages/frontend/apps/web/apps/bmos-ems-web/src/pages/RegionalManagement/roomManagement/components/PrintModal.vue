...
<script lang="tsx" setup>
  ...
  import {
    ...
    postTagInstancePrintBatch, //打印
  } from '@/services';
  import { message } from 'ant-design-vue';

  const props = withDefaults(
    defineProps<{
      operationSelectedRows?: any;
    }>(),
    {
    ...
    const schemas = [
      {
        ...
        componentProps: {
          ...
          request: async () => {
            const res: any = await reqGetPrintEquipment();
            return res.data || [];
          },
          onChange: (val, option) => {
  ...
  const modifyHtml = () => {
    ...
    // console.log(useFrom, 'useFromuseFrom');
    const valueInfo = [
      {
        field: 'fullName',
        label: t('物料全称'),
    ...
    [
      ...
      const value: any[] = valueInfo.filter((valueInfo: any) => valueInfo.field === item.dataSourceField);
  ...
  const ok = async () => {
    ...
    try {
      const batchParams = props.operationSelectedRows.map((item: any) => {
        return {
