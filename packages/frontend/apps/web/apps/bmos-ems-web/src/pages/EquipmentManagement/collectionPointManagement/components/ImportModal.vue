  ...
  <BMModalForm
    ...
    <div v-show="current === 2" class="step">
      <div v-if="success" class="step3">
        <Image :src="successImg" width="40px" height="40x" :preview="false"></Image>
        <div style="margin-left: 18px">
          <div class="infoTip1">{{ t('批量导入已完成') }}</div>
          <!-- <div class="infoTip2">{{ t('已成功导入') }} {{ count }} {{ t('条') }}</div> -->
        </div>
      </div>
      <!-- 导入失败 -->
      <div v-else class="step3">
        <Image :src="failImg" width="40px" height="40x" :preview="false"></Image>
        <div style="margin-left: 18px">
...
<script lang="ts" setup>
  ...
  import { message, Button, Steps, Upload, Image, Progress } from 'ant-design-vue';
  import { reqAcquisitionPointTemplate, reqAcquisitionPointImport } from '@/services';
  import { InfoCircleOutlined } from '@ant-design/icons-vue';
  import { fileStreamDownload } from '@bmos/utils';
  import type { UploadProps } from 'ant-design-vue';
  import successImg from '/src/assets/img/importSuccess.png';
  import failImg from '/src/assets/img/importFail.png';

  const emit = defineEmits(['updateTable']);
  const modalFormRef = ref<ModalFormInstance>();
  const open = ref<boolean>(false);
  const current = ref<number>(0);
  // const count = ref<any>(123);
  ...
  const next = async () => {
    ...
    setTimeout(() => {
      ...
      reqAcquisitionPointImport(formData).then((res: any) => {
  ...
  const downloadFile = async () => {
    try {
      const res = await reqAcquisitionPointTemplate();
      const fileName: any = getContentBetweenChars(res.headers['content-disposition']);
      fileStreamDownload(res.data, fileName);
    } catch (error: any) {
