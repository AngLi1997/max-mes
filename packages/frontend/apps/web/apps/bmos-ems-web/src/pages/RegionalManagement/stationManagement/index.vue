...
<template>
  <BMPageComponent
    ...
    :titles="[t('工位列表')]"
    :showAction="
      hasPermission('160030003000001') || hasPermission('160030003000002') || hasPermission('160030003000003')
    "
    :fieldNames="{
      title: 'showName',
      key: 'id',
    ...
    <template #tableHeaderToolbar0="{ treeNode }">
      ...
      <Button v-hasAuth="160030003000004" type="primary" @click="handleWorkstation(treeNode, 'add')">
        {{ t('新增工位') }}
      </Button>
    </template>
  </BMPageComponent>
...
<script setup lang="tsx">
  ...
  import {
    reqStationModuleTreeList, //工位树
    reqStationModuleSave, //树新增
    reqStationModuleUpdate, //树修改
    reqStationModuleDelete, //树删除
  ...
  const actionList = [
    {
      ...
      action: 'addChildren',
      ifShow: () => {
        return hasPermission('160030003000001');
      },
    },
    {
      title: t('编辑分类'),
      action: 'editNode',
      ifShow: () => {
        return hasPermission('160030003000002');
      },
    },
    {
      title: t('删除分类'),
      action: 'deleteNode',
      ifShow: () => {
        return hasPermission('160030003000003');
      },
    },
  ];
  // 表格项
  const columns: TableColumn[] = [
    ...
    },
    {
      ...
      actions: ({ record }, tableAction) => [
        {
          label: t('查看'),
          ifShow: record.enable && hasPermission('160030003000006'),
          onClick: () => {
        ...
        {
          label: t('编辑'),
          ifShow: !record.enable && hasPermission('160030003000005'),
          onClick: () => {
        ...
        {
          label: t('绑定设备'),
          ifShow: hasPermission('160030003000007'),
          onClick: () => {
            rowId.value = record.id;
        ...
        {
          label: t('绑定人员'),
          ifShow: hasPermission('160030003000008'),
          onClick: () => {
            rowId.value = record.id;
        ...
        {
          label: t('删除'),
          ifShow: !record.enable && hasPermission('160030003000009'),
          danger: true,
          onClick: () => {
            Modal.confirm({
              title: t('删除确认'),
  ...
  const deleteNodeFn = (node: any) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await reqStationModuleDelete(node.id);
          message.success(t('删除成功'));
          getTreeData();
          if (node.selected) {
    ...
    instance.validate().then(async params => {
      ...
      try {
        if (data.id) {
          ...
          await reqStationModuleUpdate(params2 as any);
        } else {
          // 新增保存
          const data2 = {
            ...data,
          };
          await reqStationModuleSave(data2);
        }
