...
<script setup lang="ts">
  ...
  import { reqRoomTreeRoom, reqLineBindRoom } from '@/services';
  ...
  const getTreeData = async () => {
    try {
      const res = await reqRoomTreeRoom();
      tree.treeData = [
        {
          name: t('全部'),
          id: '0',
  ...
  const check = (
    ...
    checkedIds.value = [];
    const temp = info.checkedNodes.filter((item: any) => item.roomFlag == true);
  ...
  const ok = async () => {
    ...
    try {
      const data = {
        id: props.rowId,
        roomIdList: checkedIds.value,
      };
      await reqLineBindRoom(data);
  ...
  watch(
    ...
    async val => {
      ...
      if (val) {
        getTreeData();
        tree.CHECKED_KEYS = props.roomIdList;
        checkedIds.value = props.roomIdList;
      } else {
        tree.CHECKED_KEYS = [];
      }
    },
