...
<script setup lang="ts">
  ...
  const queryWarp = () => {
    ...
    const tar = headerTabsREF.value?.$el?.querySelector('.ems-tabs-nav-wrap');
    if (!tar) return;
...
<style scoped lang="css">
  ...
  :deep(.header-tabs .ems-tabs-nav) {
    margin-bottom: 0;
  }

  :deep(.header-tabs .ems-tabs-tab) {
    border: none;
  }
  :deep(.header-tabs .ems-tabs-nav-operations) {
    display: none;
  }
</style>
