...
<script setup lang="ts">
  ...
  import { getPlasmaInStorageDetailList } from '@/services';
  ...
  const loadData = async (params: any, onChangeParams: any): Promise<any> => {
    ...
    return await getPlasmaInStorageDetailList(datas);
  };
  ...
  const syncBatchNo = ref<string>('');

  const showDrawer = (data: any) => {
    syncBatchNo.value = data.syncBatchNo;
    open.value = true;
  };
