import { getPlasmaAmalgamationList } from '@/services';
...
export const useDubTable = () => {
  ...
  const cancelMerge = (data: any) => {
    scanList.value = scanList.value.filter((item: any) => item !== data.plasmaOrgNo);
  };
  ...
  const leftLoadData = async (params: any) => {
    ...
    const datas = {
      ...params,
      addPlasmaOrgNoList: scanList.value,
    };
    ...
    const { data } = await getPlasmaAmalgamationList(datas);
    containerType.value = data?.[0]?.containerNoType?.value ?? null;

    // 对data进行排序，在scanList中的数据在后面
    data?.sort((a: any, b: any) => {
      if (scanList.value.includes(a.plasmaOrgNo)) {
        return 1;
      }
      if (scanList.value.includes(b.plasmaOrgNo)) {
        return -1;
      }
      return 0;
    });
          ...
          actions: ({ record }) => [
            {
              ...
              ifShow: scanList.value?.includes(record.plasmaOrgNo),
              onClick: () => {
                // look(record);
                cancelMerge(record);
                fetchDubData();
  ...
  const rightLoadData = async (params: any) => {
    const datas = {
      ...params,
      decPlasmaOrgNoList: scanList.value,
    };
    if (!datas.containerNo) {
  ...
  const isFull = () => {
    if (containerType.value == 1) {
      ...
      return leftTableData.value.length >= 8;
    }
  };
