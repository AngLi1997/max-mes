  ...
  <BMPageComponent
    ...
    <template #tableHeaderTitle0>
      <div class="table-header">
        ...
        <Button v-hasAuth="170020001000004" :disabled="disCancelSync()" @click="cancelSync(operationSelectedRows)">
          {{ t('撤销同步') }}
        </Button>
      </div>
    </template>
    <template #tableHeaderToolbar0>
      <div class="table-header">
        ...
        <Button v-hasAuth="170020001000002" @click="enterImport">
          {{ t('导入') }}
        </Button>
      </div>
    </template>
...
<script setup lang="ts">
  import { getSampleDataSyncList, sampleDataSyncRevocation, sampleDataSyncExport } from '@/services';
  ...
  const exportExcel = async (data: any) => {
    try {
      loading.value = true;
      const res = await sampleDataSyncExport({ syncId: data.id });
  ...
  const enterImport = () => {
    // emit('enterImport');
    router.push({ name: 'SampleDataSyncImportExcel' });
  };

  const loadData: DataRequestFn = async (params: any): Promise<any> => {
    const datas = { ...params };
    return getSampleDataSyncList(datas);
  };
  ...
  const disReceive = () => {
    return (
      ...
      operationSelectedRows.value?.some((item: any) => item?.receiveStatus?.value != 0)
    );
  };
  ...
  const disCancelSync = () => {
    return (
      operationSelectedRows.value?.length === 0 ||
      operationSelectedRows.value?.some((item: any) => !(item?.syncType?.value == 2 && item?.receiveStatus?.value == 1))
    );
  };
  ...
  const cancelSync = async (rows: any) => {
    Modal.confirm({
      ...
      async onOk() {
        try {
          await sampleDataSyncRevocation({
            syncBatchNoList: rows.map((item: any) => item.syncBatchNo),
          });
  ...
  const { pageRef, columnsFirst, formFirstProps } = useTable(openReceive, cancelSync, openCount);
</script>
