import {
  getWmsUnqualifiedPlasmaInList,
  getWmsUnqualifiedPlasmaOutList,
  unqualifiedPlasmaInWarehouseIn,
} from '@/services';
...
export const useDubTable = (openOutputModal: any) => {
  ...
  const leftLoadData = async (params: any): Promise<any> => {
    ...
    const res = await getWmsUnqualifiedPlasmaOutList(datas);
          ...
          actions: ({ record }) => [
            {
              ...
              ifShow: hasPermission('170090002000002') && record?.storageStatus != 1,
              onClick: () => {
                Modal.confirm({
                  title: t('是否对该血浆进行回库'),
                  ...
                  async onOk() {
                    try {
                      await unqualifiedPlasmaInWarehouseIn(record.batchNo);
  ...
  const rightLoadData = async (params: any): Promise<any> => {
    ...
    const res = await getWmsUnqualifiedPlasmaInList(datas);
          ...
          actions: ({ record }) => [
            {
              label: t('合并出库'),
              // ifShow: !record.confirmStatus.value && hasPermission('100020006000008'),
              ifShow: hasPermission('170090002000001') && record?.storageStatus == 2,
              onClick: () => {
