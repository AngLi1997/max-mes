...
<template>
  <BMPageComponent
    ...
    :rowClassNames="[
      (data: any) => {
        return data.remainingTime === 0 ? 'row-color' : '';
      },
    ]"
    :bordereds="[true]"
    :requests="[getPlasmaStockInventoryWarningList as DataRequestFn]"
    :paginations="[paginationFirst]"
    :columns="[columnsFirst]"></BMPageComponent>
</template>

<script setup lang="ts">
  import { getPlasmaStockInventoryWarningList } from '@/services';
