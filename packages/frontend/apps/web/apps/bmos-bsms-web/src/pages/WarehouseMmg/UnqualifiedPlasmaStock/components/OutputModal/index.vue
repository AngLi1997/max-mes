...
<script setup lang="ts">
  ...
  import { unqualifiedPlasmaOutWarehouseOut } from '@/services';
  ...
  const request = async (formModal: any) => {
    try {
      const params = {
        ...
        bigContainerNo: info.value.bigContainerNo,
        ...formModal,
      };
      return await unqualifiedPlasmaOutWarehouseOut(params);
    } catch (error: any) {
  ...
  const submit = async (formModal: any) => {
    try {
      ...
      emits('submitSuccess', info.value);
      cancel();
    } catch (error: any) {
