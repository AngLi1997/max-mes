import {
  ...
  getSortingPlasmaSortedList,
  getSortingPlasmaToSortList,
  sortingPlasmaRevocation,
} from '@/services';
...
export const useDubTable = () => {
  ...
  const topLoadData = async (params: any, onChangeParams: any): Promise<any> => {
    ...
    const res = await getSortingPlasmaToSortList(datas);
  ...
  const bottomLoadData = async (params: any, onChangeParams: any) => {
    ...
    if (!datas.checkNo || !datas.sortingBatchNo) {
      return {
        data: [],
      };
    }
    return await getSortingPlasmaSortedList(datas);
  };
          ...
          actions: ({ record }) => [
            {
              ...
              onClick: () => {
                Modal.confirm({
                  ...
                  async onOk() {
                    try {
                      await sortingPlasmaRevocation(record.plasmaOrgNo);
