...
<script setup lang="tsx">
  ...
  import { getAppearanceDetail } from '@/services';
  ...
  const cardItemsCmp = computed(() => {
    return status.value?.value ? cardItems.value : [cardItems.value[0], cardItems.value[cardItems.value.length - 1]];
  });

  onMounted(async () => {
    try {
      if (route.params?.plasmaOrgNo) {
        const { data } = await getAppearanceDetail(route.params.plasmaOrgNo as string);
        info.value = {
          ...data.baseInfo,
          ...data.inventoryInfo,
          ...data.applyInfo,
