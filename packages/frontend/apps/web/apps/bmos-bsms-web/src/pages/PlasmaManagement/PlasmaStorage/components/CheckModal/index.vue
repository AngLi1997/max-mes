...
<script setup lang="ts">
  ...
  import { getPlasmaInStorageVerifyList, plasmaInStorageVerifyScan } from '@/services';
  ...
  const loadData: DataRequestFn = async (params, onChangeParams): Promise<any> => {
    ...
    const { data } = await getPlasmaInStorageVerifyList(datas);
    cntObj.value = {
  ...
  const scanFn = async () => {
    try {
      const res = await plasmaInStorageVerifyScan({ syncBatchNo: batchNo.value, plasmaOrgNo: scanNo.value });
      scanNo.value = '';
