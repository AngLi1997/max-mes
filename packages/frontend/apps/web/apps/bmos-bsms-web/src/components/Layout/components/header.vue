...
<script setup lang="ts">
  ...
  const queryWarp = () => {
    ...
    const tar = headerTabsREF.value?.$el?.querySelector('.bsms-tabs-nav-wrap');
    if (!tar) return;
...
<style scoped lang="css">
  ...
  :deep(.header-tabs .bsms-tabs-nav) {
    margin-bottom: 0;
  }

  :deep(.header-tabs .bsms-tabs-tab) {
    border: none;
  }
  :deep(.header-tabs .bsms-tabs-nav-operations) {
    display: none;
  }
</style>
