...
<script setup lang="ts">
  ...
  // 输入框值变化,限制输入的值
  const hexChange = (e: { target: { value: any } }) => {
  ...
  const redChange = (e: { target: { value: any } }) => {
    let v = e.target.value;
    if (v !== '') {
      v > 255 && (red.value = 255);
      v < 0 && (red.value = 0);
  ...
  const greenChange = (e: { target: { value: any } }) => {
    let v = e.target.value;
    if (v !== '') {
  ...
  const blueChange = (e: { target: { value: any } }) => {
    let v = e.target.value;
    if (v !== '') {
  ...
  const alphaChange = (e: { target: { value: any } }) => {
    let v = e.target.value;
    if (v !== '') {
      v = parseFloat(v);
      alpha.value = v;
