...
<script setup lang="ts">
  ...
  const queryWarp = () => {
    ...
    const tar = headerTabsREF.value?.$el?.querySelector('.wms-tabs-nav-wrap');
    if (!tar) return;
...
<style scoped lang="css">
  ...
  :deep(.header-tabs .wms-tabs-nav) {
    margin-bottom: 0;
  }

  :deep(.header-tabs .wms-tabs-tab) {
    border: none;
  }
  :deep(.header-tabs .wms-tabs-nav-operations) {
    display: none;
  }
</style>
