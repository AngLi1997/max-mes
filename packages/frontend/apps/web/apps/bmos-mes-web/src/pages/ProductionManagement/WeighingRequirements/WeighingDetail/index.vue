  ...
  <BreadcrumbButton :loading>
    <template #breadcrumb>
      <Breadcrumb class="mes-breadcrumb">
        ...
        <breadcrumb-item>{{ t('称量详情') }}</breadcrumb-item>
      </Breadcrumb>
    </template>
    <template #btns>
...
<script setup lang="tsx">
  ...
  import { reqWeighingRequirementsGetWeighRecord } from '@/services';
  ...
  const loadTableData = async () => {
    try {
      loading.value = true;
      const { data } = await reqWeighingRequirementsGetWeighRecord({
        groupId: requireId.value,
      });
      myFormRef.value?.setFieldsValue({
        ...route.query,
        productName: `${route.query.mergeCode}-${route.query.materialName}`,
      });
