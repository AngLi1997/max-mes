import {
  getPlanProcessList,
  reqLotReleaseListVersionByTemplateId,
...
import { isNullOrUnDef, loopSelectableTree } from '@bmos/utils';
    ...
    schemas: [
      {
        ...
        componentProps: {
          disabled: true,
          request: async () => {
            return [{ label: props.formValue?.name, value: props.formValue.lotReleaseTemplateId }];
          },
        },
      },
      {
        field: 'lotReleaseVersion',
        component: 'Select',
        label: t('批签发版本'),
        required: true,
        componentProps: {
          disabled: true,
          request: async () => {
            return [{ label: props.formValue?.lotReleaseVersion, value: props.formValue.lotReleaseVersion }];
          },
        },
      },
      {
        ...
        componentProps: ({ formModel }: RenderCallbackParams) => {
          return {
            ...
            request: async () => {
              try {
                const { data } = await reqLotReleaseManageProductTree(props.formValue.lotReleaseTemplateId);
                return loopSelectableTree(data, 'categoryFlag', true);
              } catch (error) {}
            },
            onChange: () => {
              formModel.processId = undefined;
              formModel.planId = undefined;
            },
          };
        },
      },
      ...
      },
      {
        ...
        componentProps: ({ formModel }: RenderCallbackParams) => {
          return {
            ...
            request: async () => {
              try {
                if (!props.formValue?.lotReleaseTemplateId) {
                  return [];
                }
                const { data } = await reqLotReleaseListVersionByTemplateId(props.formValue?.lotReleaseTemplateId);
                if (isNullOrUnDef(formModel.lotReleaseVersion) && data?.length) {
                  formModel.lotReleaseVersion = data?.find((item: any) => item.isDefault)?.version;
                }
                return data;
              } catch (error) {
                return [];
