...
<script setup lang="ts">
  ...
  import { reqEquipmentStationTree, reqWeighCentreBindStation, reqWeighCentreQueryInfo } from '@/services';
  ...
  const checkedIds = ref<any>([]); //工位id集合
  ...
  const ok = async () => {
    ...
    try {
      ...
      await reqWeighCentreBindStation(data);
  ...
  watch(
    ...
    async val => {
      ...
      if (val) {
        getTreeData();
        const { data } = await reqWeighCentreQueryInfo({ id: props.rowId });
        tree.CHECKED_KEYS = data.stationIds;
        checkedIds.value = data.stationIds;
      } else {
        tree.CHECKED_KEYS = [];
      }
    },
