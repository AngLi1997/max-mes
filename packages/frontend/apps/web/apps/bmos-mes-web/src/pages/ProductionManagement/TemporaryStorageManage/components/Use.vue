...
<script lang="tsx" setup>
  ...
  import { reqStorageMaterialUseAndConsumeMobile } from '@/services';
  ...
  const request = async () => {
    try {
      await reqStorageMaterialUseAndConsumeMobile({
        ...curFormModal.value,
      });
      emit('updateTable');
      message.success(t('使用成功'));
      open.value = false;
    } catch (error: any) {
  ...
  const submit = async (formModal: Recordable) => {
    try {
      await getPositionUserList('120030008000024');
      signOpen.value = true;
