...
export const useTree = () => {
  ...
  const actionList = [
    {
      ...
      ifShow: () => {
        return hasPermission('120020010000001');
      }
    },
    {
      title: t('编辑分类'),
      ...
      ifShow: () => {
        return hasPermission('120020010000002');
      }
    },
    {
      title: t('删除分类'),
      ...
      ifShow: () => {
        return hasPermission('120020010000003');
      },
  ...
  const treeAction = (action: any, node: any) => {
    ...
    SubmitKey.value =
      action?.action == 'ADD' ? modelType.addTree : action?.action;
    if (textType.includes(action.action)) {
      treeFormProps.value.initialValues!.name = node.data.name || '';
      treeFormProps.value.initialValues!.parentId = node.data?.parentId
        ? node.data.parentId
        : 'all';
    }
    modalTitle.value =
      action?.action == modelType.editTree
        ? t('编辑存储区域')
        : t('新增存储区域');
    status.value = true;
  };
