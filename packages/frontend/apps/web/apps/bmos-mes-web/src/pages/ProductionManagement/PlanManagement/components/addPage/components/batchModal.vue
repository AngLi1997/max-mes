    ...
    <div class="tree-container">
      ...
      <div v-show="segmentedValue === t('历史批次')">
        ...
        <CheckboxGroup v-model:value="checkboxValue1" class="group">
          <div v-for="(item, index) in list1" :key="index" style="width: 100%; margin-bottom: 10px">
...
<script setup lang="ts">
  ...
  const searchKey2 = ref<any>(''); //计划批次输入框
  const checkboxValue0 = ref<any>([]); //勾选的框
  const checkboxValue1 = ref<any>([]);
  const checkboxValue2 = ref<any>([]); //计划批次勾选框
  ...
  const ok = () => {
    ...
    const batchNoList = [...checkedNodes0.value, ...checkedNodes1.value]?.map((item: any) => item.batchNo);

    // const temp2=temp//关联批次和历史批次的批号
    sorts = checkedNodes2.value.map((item: any) => item.sort); //计划批次的下标

    emit('modalOk', planIds, batchNos, props.rowData?.id, checkedNodes1.value, sorts, batchNoList);
    open.value = false;
  };
  ...
  // 获取对应的计划批次并回显
  const getList2 = async (processId: any) => {
    list2.value = props.relatedPlanBatch
      ?.filter(item0 => item0.processId == processId)
      ?.map((item: any) => {
        return {
          ...item,
          id: item.sort,
      ...
      .filter(item2 => item2.batchNo && item2.planNo); //有计划批次有批号和编号才展示
  ...
  watch(
    ...
    async val => {
      ...
      if (val) {
        ...
        getList0(props.rowData.id);
        getList2(props.rowData.id);
