...
export const useVirtualized = (tableData: ModelRef<any>) => {
  const yItemHeight = ref(58);
  //表格特有class名生成
  ...
  const scrollTo = (fieldId: string) => {
    const index = tableData.value.findIndex((item: any) => item.fieldId === fieldId);
    if (index !== -1) {
  ...
  watch(
    ...
    val => {
      if (val && val[1] >= range.value[1] && (!parentNode.value || !contentNode.value)) {
        initVir();
      }
      if (val && val[1] && parentNode.value && contentNode.value) {
        scrollY.value = parentNode.value.clientHeight;
        let placeholderHeight = yItemHeight.value * tableData.value.length - scrollY.value;
        if (placeholderHeight < 0) {
          placeholderHeight = 0;
