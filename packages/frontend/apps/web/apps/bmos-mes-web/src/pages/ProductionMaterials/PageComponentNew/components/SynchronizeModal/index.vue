...
<script setup lang="tsx">
  ...
  import { postMaterialSyncApi, getPlatformMaterialTreeApi, getProductMaterialSyncTreeAllApi } from '@/services';
  ...
  const getDefaultTreeData = async () => {
    ...
    try {
      if (activeKey.value === '1') {
        ...
        const res: any = await getProductMaterialSyncTreeAllApi();
        leftTreeData.value = [{ id: '0', showName: t('全部'), children: res.data || [] }] as any;
      }
    } catch (error) {
      message.error(error.message);
