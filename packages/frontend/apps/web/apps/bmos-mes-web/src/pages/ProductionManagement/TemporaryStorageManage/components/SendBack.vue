...
<script lang="tsx" setup>
  ...
  import { reqStorageMaterialSendBackAndConsumeMobile } from '@/services';
  ...
  const request = async () => {
    try {
      await reqStorageMaterialSendBackAndConsumeMobile({
        ...curFormModal.value,
      });
      emit('updateTable');
      message.success(t('退库成功'));
      open.value = false;
    } catch (error: any) {
  ...
  const submit = async (formModal: Recordable) => {
    try {
      await getPositionUserList('120030008000022');
      signOpen.value = true;
