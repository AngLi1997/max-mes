import { reqPlanInfoRelationPlanList } from '@/services';
import { BMIcon, FormProps, Recordable, TableColumn } from '@bmos/components';
...
export const useStepTwo = ({ stepOneFormValue }: UseStepTwoParams) => {
  const stepTwoRef = ref<any>();

  const stepFormValues = ref<any>({});
  const stepTwoFormProps = reactive<FormProps>({
  ...
  const stepTwoTableColumns: Ref<TableColumn[]> = ref([
    ...
    },
    {
      ...
      customRender: ({ record }) => {
        return `${record.processName}--${record.processVersion}`;
      },
    },
    {
      title: t('生产批号'),
  ...
  const getStepTwoTableData = async () => {
    try {
      ...
      const { data } = await reqPlanInfoRelationPlanList(stepOneFormValue.value?.planId);
      stepTwoTableDataSource.value = data.map((item: any, index: number) => {
        return {
          ...item,
          index: index + 1,
        };
