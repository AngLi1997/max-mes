...
export type UseCleanImplementConfigParams = {
  props: ConfigFormProps;
  hasChange: Ref<boolean>;
};
// 清场执行
export const useCleanImplementConfig = ({ props, hasChange }: UseCleanImplementConfigParams) => {
  const { stationConfig } = useCommonStationConfig({ props, hasChange });
  const cleanImplementConfig = ref<FormSchema[]>([
    {
