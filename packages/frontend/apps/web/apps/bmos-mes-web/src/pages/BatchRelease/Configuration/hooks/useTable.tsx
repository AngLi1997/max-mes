import {
  reqLotReleaseTemplateCategoryCreateCategory,
  reqLotReleaseTemplateCategoryDelete,
  reqLotReleaseTemplateCategoryEditCategory,
  reqLotReleaseTemplateCategoryTree,
  reqLotReleaseTemplateDownloadTemplate,
  reqLotReleaseTemplateMakeDefault,
  reqLotReleaseTemplateMakeSure,
  reqLotReleaseTemplateQueryListProcessIdByTemplateId,
  reqLotReleaseTemplateQueryPage,
  reqLotReleaseTemplateQueryVersionPage,
  reqLotReleaseTemplateScrap,
} from '@/services';
...
export const useTables = () => {
  ...
  const getCheckedProcessIds = async (id: string) => {
    try {
      const { data } = await reqLotReleaseTemplateQueryListProcessIdByTemplateId(id);
  ...
  const templateColumn: TableColumn[] = [
    ...
    },
    {
      ...
      actions: ({ record }) => [
        {
          label: t('数据权限'),
          ifShow: hasPermission('120040002000005'),
          onClick: () => {
        ...
        {
          label: t('绑定工艺'),
          ifShow: hasPermission('120040002000006'),
          onClick: () => {
            firstRowData.value = record;
            getCheckedProcessIds(record.id);
          },
        },
      ],
    },
  ...
  const confirmVersion = async (record: Recordable) => {
    Modal.confirm({
      ...
      content: t('确认后此版本的批签发将无法修改'),
      async onOk() {
        try {
          await reqLotReleaseTemplateMakeSure(record.id);
  ...
  const voidVersion = async (record: Recordable) => {
    Modal.confirm({
      ...
      async onOk() {
        try {
          await reqLotReleaseTemplateScrap(record.id);
          message.success(t('操作成功'));
          updateSecondTable();
          return Promise.resolve();
        } catch (error: any) {
  ...
  const setDefaultVersion = async (record: Recordable) => {
    Modal.confirm({
      ...
      async onOk() {
        try {
          await reqLotReleaseTemplateMakeDefault(record.id);
          message.success(t('操作成功'));
          updateSecondTable();
          return Promise.resolve();
        } catch (error: any) {
  ...
  const downloadFile = async (record: any) => {
    try {
      const res = await reqLotReleaseTemplateDownloadTemplate(record.id);
      fileStreamDownload(
        res,
        `${record.templateName ? record.templateName : ''}--${record.version}.${record.templateUrl.split('.').pop()}`,
      );
    } catch (error) {}
  };
  ...
  const listColumn: TableColumn[] = [
    ...
    },
    {
      ...
      actions: ({ record }) => [
        {
          label: t('上传'),
          ifShow: hasPermission('120040002000008') && record.status?.value === StatusType.EDIT,
          onClick: () => {
        ...
        {
          label: t('测试'),
          ifShow: hasPermission('120040002000010') && record.status?.value !== StatusType.SCRAP,
          onClick: () => {
            secondRowData.value = record;
            stepOpen.value = true;
          },
        ...
        {
          label: t('确认'),
          ifShow: hasPermission('120040002000011') && record.status?.value === StatusType.EDIT,
          onClick: () => {
        ...
        {
          label: t('下载'),
          ifShow: hasPermission('120040002000009'),
          onClick: () => {
            downloadFile(record);
          },
        },
        {
          label: t('设为默认'),
          ifShow: hasPermission('120040002000014') && record.status?.value === StatusType.MAKE_SURE,
          onClick: () => {
        ...
        {
          label: t('历史'),
          ifShow: hasPermission('120040002000012'),
          onClick: () => {
        ...
        {
          label: t('作废'),
          ifShow: hasPermission('120040002000013') && record.status?.value !== StatusType.SCRAP,
          danger: true,
          onClick: () => {
            voidVersion(record);
          },
  ...
  const getTreeData = async () => {
    try {
      const { data } = await reqLotReleaseTemplateCategoryTree();
      treeData.value = [
        {
          id: 'all',
          name: t('全部'),
  ...
  const deleteNodeFn = (node: DataNode) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await reqLotReleaseTemplateCategoryDelete(node.id);
  ...
  const actionList: ActionListItem[] = [
    {
      ...
      ifShow: (node: ActionListItemCustomRenderParams) => {
        return node.nodeLevelInTree < 7 && hasPermission('120040002000001');
      },
    },
    {
      title: t('编辑分类'),
      action: 'editNode',
      ifShow: () => hasPermission('120040002000002'),
    },
    {
      title: t('删除分类'),
      action: 'deleteNode',
      ifShow: () => hasPermission('120040002000003'),
    },
  ];
  ...
  const treeModalSubmit = async (values: Recordable) => {
    try {
      if (treeOperation.value === OperationType.Add) {
        await reqLotReleaseTemplateCategoryCreateCategory({
          name: values.name,
          parentId: values.parentId === 'all' ? null : values.parentId,
        });
        message.success(t('新增成功'));
      } else {
        await reqLotReleaseTemplateCategoryEditCategory({
          id: values.id,
          name: values.name,
        });
  ...
  const getDatasetPageList = async (params: any) => {
    if (!params.categoryId || params.categoryId === 'all') {
      return await reqLotReleaseTemplateQueryPage({
        ...params,
        categoryId: undefined,
      });
    }
    return await reqLotReleaseTemplateQueryPage(params);
  };

  const getDatasetVersionPageList = async (params: any) => {
    if (!params.logReleaseTemplateId) return Promise.resolve({ data: [] });
    return await reqLotReleaseTemplateQueryVersionPage(params);
  };
