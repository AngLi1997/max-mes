...
export const useEventListener = (flowEventListenersContext: useEventListenerParams) => {
  const { emit, showPorts, graph, isCustomNode, isNotStartOrEndCell, props, isView } = flowEventListenersContext;
  const setEventListener = () => {
    ...
    graph.value.on('cell:removed', ({ cell }) => {
      ...
      if (cell.shape === 'edge') {
        ...
        // 如果是节点，删除节点
        const edges = flowData.cells.filter(item => item.shape === 'edge' && item.source?.cell === cell.id);
        const targets: any = [];
        edges.forEach(item => {
          const target = flowData.cells.find(cell => cell.id === item.target?.cell);
          if (target && target.shape?.includes('gateway')) {
            targets.push(target);
          }
        });
