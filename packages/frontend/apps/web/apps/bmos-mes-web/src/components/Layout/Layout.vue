<script setup lang="ts">
  ...
  import { handleMenuList } from './index';
  ...
  const notKeepAlive = ['InstructionListDecomposition'];
  const isKeepAlive = computed(() => {
    return route.meta.keepAlive && !notKeepAlive.includes(route.name as string);
  });
  ...
  <Layout class="mes-layout">
    <LayoutSider v-show="!hasHidden" v-model:collapsed="collapsed" :style="{ background: '#103566' }">
      <Menu v-model:openKeys="openKeys" mode="inline" :selectedKeys="selectedKeys" theme="dark" @click="menu_click">
        <BmSubMenu v-for="item in menuList" :key="item.path" :menu="item"></BmSubMenu>
      </Menu>
    </LayoutSider>
    <Layout style="flex: 1">
      ...
      <LayoutContent>
        <div class="mes-content">
          <router-view v-slot="{ Component }">
            <keep-alive :max="1">
              <component :is="Component" v-if="isKeepAlive" :key="currentKey" />
            </keep-alive>
            <component :is="Component" v-if="!isKeepAlive" :key="currentKey" />
          </router-view>
        </div>
...
<style scoped>
  ...
  /* 如果 mes-content 的 儿子节点为 bread-crumb-container 就不要padding */
  .mes-content:has(.bread-crumb-container) {
    padding: 0;
  }
  .mes-layout {
    height: 100%;
  ...
  :deep(.mes-layout-sider-children) {
    overflow-y: auto;
  }
  :deep(.mes-layout-sider-children)::-webkit-scrollbar {
    width: 0;
  }
</style>
