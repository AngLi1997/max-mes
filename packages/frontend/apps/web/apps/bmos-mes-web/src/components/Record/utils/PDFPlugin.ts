...
class PDFPlugin {
  ...
  contentWidth: number = 550;
  distance: Distance | undefined;
  constructor(title: string) {
    this.JS_PDF = new JsPDF({
      unit: 'pt',
      format: 'a4',
      orientation: 'p',
    });
  ...
  async add(
    ...
    await htmlPdf.getPage(html, this.JS_PDF, this.header, this.footer, {
  ...
  private async addPageHeader(header: string | HTMLElement, pattern: number = 1) {
    ...
    await createPageByHtml(asHeader as HTMLElement, {
      header: this.headerHeight,
      headerW: this.headerWidth,
      footer: this.footerHeight,
      footerW: this.footerWidth,
  ...
  private async addPageFooter(footer: string | HTMLElement, pattern: number = 1) {
    ...
    if (typeof asFooter === 'string') {
      const { par, div } = createHeaderOrFooter(asFooter as unknown as string, pattern, this.distance!);
      const { width, height } = div.getBoundingClientRect();
    ...
    await createPageByHtml(asFooter, {
      header: this.headerHeight,
      headerW: this.headerWidth,
      footer: this.footerHeight,
      footerW: this.footerWidth,
