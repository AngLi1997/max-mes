...
import { cloneDeep, deepMerge, isEmpty } from '@bmos/utils';
import { message } from 'ant-design-vue';
import { Ref } from 'vue';
import { FlowEmitFn, FlowNodeEnum, FlowNodeEnumType } from '../type';
...
export const useFlowMethods = (flowMethodsContext: useFlowMethodsParams) => {
  ...
  const isNotGateway = (cells: Cell<Cell.Properties>[]) => {
    if (cells.length && cells[0].shape !== FlowNodeEnum.GATEWAY) {
      return true;
    } else {
      return false;
    }
   ...
   */
  const getFlowData = (_needTransformData = true) => {
    try {
   ...
   * @param {Cell} cell 节点
   */
  const handleClickNext = (cell: Cell, shape: FlowNodeEnumType) => {
    emit('handleClickNext', cell, shape);
  };
  /**
  ...
  const updateFormValue = (settingNodeId: string, formValue: Recordable) => {
    ...
    let obj: Recordable = cloneDeep(cellData);
    obj = deepMerge(isEmpty(cellData) ? {} : cellData, formValue);
    if (isEmpty(obj['name'])) {
      formValue.name = obj.label;
    }
    cell?.setData(
      {
        ...obj,
        ...(formValue.name ? { label: formValue.name } : { label: obj.label }),
  ...
  const updateCellData = (settingNodeId: string, formValue: Recordable, key = 'formData') => {
    ...
    obj[key] = deepMerge(isEmpty(cellData[key]) ? {} : cellData[key], formValue);
    if (isEmpty(obj[key]['name'])) {
      formValue.name = obj.label;
    }
    cell?.setData(
      {
