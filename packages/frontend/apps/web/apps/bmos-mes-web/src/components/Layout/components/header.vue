      ...
      <Tabs
        ...
        <template #leftExtra>
          <span class="extra-icon extra-icon-left">
            <Button type="text" size="small" :disabled="extraIcon.Left" @click="scrollWrap('left')">
              <template #icon>
        ...
        <template #rightExtra>
          <span class="extra-icon extra-icon-right">
            <Button type="text" size="small" :disabled="extraIcon.Right" @click="scrollWrap('right')">
              <template #icon>
...
<script setup lang="ts">
  ...
  const queryWarp = () => {
    ...
    const tar = headerTabsREF.value?.$el?.querySelector('.mes-tabs-nav-wrap');
    if (!tar) return;
...
<style scoped lang="css">
  ...
  :deep(.header-tabs .mes-tabs-nav) {
    margin-bottom: 0;
  }

  :deep(.header-tabs .mes-tabs-tab) {
    border: none;
  }
  :deep(.header-tabs .mes-tabs-nav-operations) {
    display: none;
  }
</style>
