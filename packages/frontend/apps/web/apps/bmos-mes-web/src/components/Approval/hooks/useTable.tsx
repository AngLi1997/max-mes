...
export const useTable = ({ props, flowInstance }: UseTableParams) => {
  ...
  const getData = async () => {
    try {
      ...
      if (
        ...
        modalJson.value = JSON.parse(data?.metaInfo || '[{}]').map((item: any) => {
          const state = (data?.nodeStateList || []).find((node: any) => node.elementKey === item.key)?.state;
          return {
            ...item.metaInfo,
            data: {
              ...item.metaInfo.data,
        ...
        curAuditNode.value = (data?.nodeStateList || []).find((node: any) => node.state === 1);
        ...
        curTableData.value = data?.nodeList.filter((item: any) => item.elementKey === curAuditNode.value.elementKey);
      }
    } catch (error) {}
  };
  ...
  const setCurTableDataByNodeId = (nodeId: string) => {
    if (nodeId) {
      curTableData.value = allTableData.value.filter(item => item.elementKey === nodeId);
    } else {
      curTableData.value = [];
    }
  };
  ...
  watch(
    ...
    () => {
      if (graphIsReady.value) {
        setTimeout(() => {
          curAuditNode.value?.elementKey && flowInstance.value?.selectNodeById(curAuditNode.value.elementKey);
        }, 1000);
      }
    },
  );
