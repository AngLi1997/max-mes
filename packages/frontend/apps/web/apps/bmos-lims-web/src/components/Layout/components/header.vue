...
<script setup lang="ts">
  ...
  const queryWarp = () => {
    ...
    const tar = headerTabsREF.value?.$el?.querySelector('.lims-tabs-nav-wrap');
    if (!tar) return;
...
<style scoped lang="css">
  ...
  :deep(.header-tabs .lims-tabs-nav) {
    margin-bottom: 0;
  }

  :deep(.header-tabs .lims-tabs-tab) {
    border: none;
  }
  :deep(.header-tabs .lims-tabs-nav-operations) {
    display: none;
  }
</style>
