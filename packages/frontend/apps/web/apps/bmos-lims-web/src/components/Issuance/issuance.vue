    ...
    <Row class="header">
      <Col :span="12">
        <Breadcrumb class="crumb">
          ...
          <breadcrumb-item>{{ t('签发') }}</breadcrumb-item>
        </Breadcrumb>
      </Col>
      <Col :span="12" class="header-btn">
        <Space :size="16">
          <Button type="primary" @click="submit">{{ t('报告签发') }}</Button>
...
<script setup lang="tsx">
  ...
  import { Descriptions, DescriptionsItem, RadioGroup, Radio, message } from 'ant-design-vue';
  import { LimsCard } from '@/components/Card';
  import { getCheckOrderReportInfo, signCheckOrderReport } from '@/services/index';
  ...
  const submitSuccess = async (formModel: any) => {
    try {
      ...
      await signCheckOrderReport(data);
  ...
  const submit = () => {
    (signModalProps.title = t('报告签发')),
      (signModalProps.extraSchemas = [
        {
          ...
          component: ({ formModel }: RenderCallbackParams) => (
            <RadioGroup
              ...
              onChange={(_e: any) => {
                if (formModel.result === 'PASS') {
                  // 删除数组最后一项
                  signModalProps.extraSchemas.pop();
                } else if (formModel.result === 'NOT_PASS') {
                  // 在数组最后一项插入
                  signModalProps.extraSchemas.push(tempSchema);
                }
              }}>
      ...
      (signModalProps.signatureAction = 26);
    signOpen.value = true;
