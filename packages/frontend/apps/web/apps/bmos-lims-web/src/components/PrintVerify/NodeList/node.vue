<template>
  <div
    ...
    <div class="node-content-container">
      ...
      <span class="node-content node-content-type">{{ title }}</span>
      <span class="node-content node-content-title">No.{{ componentNumber }}</span>
    </div>
    <div v-if="showIcon && !lookup" class="node-operation">
      <FormOutlined
        v-if="edit && EDITABLE_NODE_TYPE.includes(type)"
        class="operation-icon icon-edit"
        @click.stop="editClick" />
      <CloseOutlined v-if="componentShowIconValue(type)" class="operation-icon icon-delete" @click.stop="iconClick" />
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  import { EDITABLE_NODE_TYPE, BUSINESS_PRODUCT_INFO_CHILDREN, BUSINESS_FORMULA_INFO_MATERIAL_CHILDREN } from './enum';

  const componentShowIconValue = type => {
    return (
      ['BUSINESS_PRODUCT_INFO', 'BUSINESS_FORMULA_INFO', 'BUSINESS_FORMULA_INFO_MATERIAL'].includes(type) ||
      !{ ...BUSINESS_PRODUCT_INFO_CHILDREN, ...BUSINESS_FORMULA_INFO_MATERIAL_CHILDREN }[type]
    );
  };

  const emit = defineEmits(['node-click', 'icon-click', 'edit-click']);
  // eslint-disable-next-line vue/no-reserved-props
  const props = withDefaults(defineProps<nodePropsType & { showIcon: boolean; lookup: boolean; edit?: boolean }>(), {
    title: 'No.1',
    type: '数字',
    actived: false,
    rounded: true,
