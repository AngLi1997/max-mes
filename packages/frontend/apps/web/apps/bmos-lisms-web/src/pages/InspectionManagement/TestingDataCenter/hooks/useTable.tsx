...
import { InspectionProjectEnum, InspectionResultEnum, PublishStatusEnum, StatusType } from '@/types';
      ...
      children: [
        {
          ...
          customRender: ({ record }) => {
            if (
              record.publishStatus?.value === PublishStatusEnum.TO_PUBLISH ||
              record.publishStatus?.value === PublishStatusEnum.PUBLISHED
            ) {
              return record.publishStatus.label;
            } else if (record.auditStatus?.value === PublishStatusEnum.TO_AUDIT) {
              return t('已发布');
            } else {
              return '-';
            }
