...
export const useTable = (
  ...
  const { auditStatusDict, signResultDict } = getDicts();
  // 备注弹窗相关
  ...
  const columnsFirst: TableColumn[] = [
    ...
    },
    {
      ...
      actions: ({ record }) => [
        {
          label: t('检验报告'),
          ifShow: hasPermission('210040001000002'),
          onClick: async () => {
            await print(record);
          },
        },
        {
          label: t('控制点记录'),
          ifShow: hasPermission('210040001000004') && record?.checkStatus?.value === 'CHECKED',
          onClick: async () => {
        ...
        {
          label: t('审核'),
          ifShow: hasPermission('210040001000001') && record?.auditStatus?.value === 'TO_AUDIT',
          onClick: () => {
            openAudit([record]);
          },
        },
