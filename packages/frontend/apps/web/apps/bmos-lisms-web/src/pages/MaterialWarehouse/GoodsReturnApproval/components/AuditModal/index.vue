...
<script setup lang="ts">
  ...
  import { materialUseReturnApprove } from '@/services';
  ...
  const submit = async (formModel: any) => {
    try {
      const params = {
        useFormIdentifyList: tableData.value.map((item: any) => item.useFormIdentify),
        ...formModel,
      };
      await materialUseReturnApprove(params);
