...
<script setup lang="ts">
  ...
  import { editMaterialInWarehouse } from '@/services';
  ...
  const openModal = async (row: any) => {
    ...
    setFormModels({
      ...
      receiveResult: row.receiveResult?.value,
      needSpotCheck: row.needSpotCheck?.value,
  ...
  const request = async (formModel: any) => {
    try {
      ...
      const { data } = await editMaterialInWarehouse(params);
      if (data) {
