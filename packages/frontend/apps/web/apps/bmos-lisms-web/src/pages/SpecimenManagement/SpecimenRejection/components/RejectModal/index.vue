...
<script setup lang="ts">
  ...
  import { getSampleRejectApply } from '@/services';
  ...
  const submit = async (formModel: any) => {
    try {
      const sampleNoList = tableData.value.map((item: any) => item.sampleNo);
      const params = {
        sampleNoList,
        sampleNo: sampleNoList.join(','),
        ...formModel,
      };
      await getSampleRejectApply(params);
