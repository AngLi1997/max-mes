...
<script setup lang="ts">
  ...
  import { materialUseSpotCheckAudit } from '@/services';
  ...
  const submit = async (formModel: any) => {
    try {
      const params = {
        ...
        inWarehouseNo: tableData.value.map((item: any) => item.inWarehouseNo).join(','),
        ...formModel,
      };
      await materialUseSpotCheckAudit(params);
