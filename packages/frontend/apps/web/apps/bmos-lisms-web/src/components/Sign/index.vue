...
<script lang="tsx" setup>
  ...
  import { mesSignatureCheckSignatureConfig, getSignatureImg, mesSignatureValidateSignaturePassword } from '@/services';
  ...
  const openSign = async (record: any) => {
    try {
      const { data: checkSignPwd } = await mesSignatureCheckSignatureConfig({
        userIds: [userInfo?.userId],
      });
      if (checkSignPwd && !checkSignPwd[0]?.result) {
        Modal.warning({
          title: t('电子签名校验'),
          icon: h(ExclamationCircleOutlined),
          content: t('请点击右上角头像，进入「签名密码设置」配置签名密码！'),
        });
  ...
  const submit = async (formValues: Recordable) => {
    try {
      const { data } = await mesSignatureValidateSignaturePassword({
        validates: [
          {
