<script setup lang="ts">
  ...
  import { cloneDeep, isUnDef } from '@bmos/utils';
  ...
  const keepAliveCmp = () => {
    return headerRef.value?.paneList.map((item: any) => item.name).filter((item: any) => !isUnDef(item));
  };
</script>

<template>
  <Layout class="lisms-layout">
    <LayoutSider v-model:collapsed="collapsed" :style="{ background: '#103566' }">
    ...
    <Layout style="flex: 1">
      ...
      <LayoutContent>
        <div class="lisms-content">
          <router-view v-slot="{ route, Component }">
            <keep-alive :include="keepAliveCmp()" :max="1">
...
<style scoped>
  ...
  :deep(.lisms-layout-sider-children) {
    overflow-y: auto;
  }
  :deep(.lisms-layout-sider-children)::-webkit-scrollbar {
    width: 0;
  }
</style>
