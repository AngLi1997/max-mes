...
<template>
  <div class="color-select">
    <div ref="saturation_value" class="saturation-value" @mousedown="mousedownSV">
    ...
    <div class="color-select-middle">
      <div style="flex: auto">
        <div ref="hue_slider" class="hue-slider" @mousedown="mousedownHue">
          <div class="slider" :style="hueSliderStyle"></div>
        </div>
        <div ref="alpha_slider" class="alpha-slider" @mousedown="mousedownAlpha">
          <div class="slider" :style="alphaSliderStyle"></div>
          <div
            :style="`background: linear-gradient(to right, rgba(0,0,0,0), ${colorObj.rgb});width: 100%;height: 100%`"></div>
        </div>
      </div>
      <div class="color-diamond">
        <div
          :style="`background-color: ${colorObj.rgba};width: 100%;height: 100%;box-shadow: inset 0 0 0 1px rgba(0, 0, 0, .15), inset 0 0 4px rgba(0, 0, 0, .25);`"></div>
      </div>
    </div>
    <div class="color-value">
      <div class="hex">
        <label>
          <input :value="colorObj.hex8" spellcheck="false" @input="hexChange" />
        </label>
        <p>Hex</p>
      </div>
      <div class="rgba-r">
...
<script setup lang="ts">
  ...
  const saturation_value = ref();
  const hue_slider = ref();
  const alpha_slider = ref();
  ...
  watch([red, green, blue], () => {
    emit('update:color', colorObj.value);
  ...
  // 点击预设方块事件
  const presetChange = (item: string) => {
  ...
  // 饱和度和亮度
  const handleChangeSV = (e: { pageX: number; pageY: number }) => {
  ...
  // 色调
  const handleChangeHue = (e: { pageX: number }) => {
  ...
  // 透明度
  const handleChangeAlpha = (e: { pageX: number }) => {
   ...
   * @param color
   */
  const parseColor = (color: string | Record<string, any>) => {
    if (color) {
      let r, g, b, a;
  ...
  const rgba2hex = (r: string | number, g: string | number, b: string | number, a = 1) => {
    if (typeof r === 'string') {
      r = parseInt(r);
    }
