...
<script setup lang="ts">
  ...
  const queryWarp = () => {
    ...
    const tar = headerTabsREF.value?.$el?.querySelector('.dc-tabs-nav-wrap');
    if (!tar) return;
...
<style scoped lang="css">
  ...
  :deep(.header-tabs .dc-tabs-nav) {
    margin-bottom: 0;
  }

  :deep(.header-tabs .dc-tabs-tab) {
    border: none;
  }
  :deep(.header-tabs .dc-tabs-nav-operations) {
    display: none;
  }
</style>
