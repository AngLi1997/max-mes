...
function setTextareaReadonly($dom, item) {
	let isReadonly = (Array.isArray(item.value) && item.value.length > 0) || item.value !== '' || item.state === 'formula'
...
function setTemplateScale(size) {
	$('#form-box').find('table').css('width', '100%');
	const obj = {
		'back-box': ['top', 'left'],
		'action-buttons': ['bottom', 'right'],
		'pagination-class': ['bottom', 'left'],
		'mark-box-class': ['top', 'right'],
	}
	Object.keys(obj).forEach(key => {
		const $dom = $(`.${key}`)
		const cssObj = {
			'transform': `scale(${size})`,
			'transform-origin': obj[key].join(' '),
		}
		obj[key].forEach(item => {
			const attr = parseFloat($dom.css(item))
			cssObj[item] = attr * (size - 0) + 'px'
		})
		$dom.css(cssObj)
	})
}
