  ...
  methods: {
    ...
    async initImage() {
      ...
      uni.getImageInfo({
        ...
        success: function (res) {
          that.$emit('load', res)
          // init image size
          that.image.originWidth = res.width
          that.image.originHeight = res.height
          that.image.width = that.fit ? that.ctrlWidth : that.image.originWidth
          that.image.height = res.height / res.width * that.image.width
          that.img = res.path

          const offset = [0, 0]
          if(that.imageCenter) {
            offset[0] = (that.ctrlWidth - that.image.width) / 2
            offset[1] = (that.ctrlHeight - that.image.height) / 2
          }
          offset[0] += that.offset[0] || 0
          offset[1] += that.offset[1] || 0
          
          that.setTranslate(offset)
          that.setZoom(that.zoom)
          that.transform.angle = that.freeBoundDetect || !that.disableRotate ? that.angle : 0

          that.setBoundary() // boundary detect
    ...
    setCut() {
      ...
      switch (this.cutDirection) {
        case 'lt':
          corner.top = (corner.top + meta.deltaY) < 0 ? 0 : corner.top + meta.deltaY
          corner.left = (corner.left + meta.deltaX) < 0 ? 0 : corner.left + meta.deltaX
          break
        case 'rt':
          corner.top = (corner.top + meta.deltaY) < 0 ? 0 : corner.top + meta.deltaY
          corner.right = (corner.right - (this.keepRatio ? -meta.deltaX : meta.deltaX)) < 0 ? 0 : (corner.right - (this.keepRatio ? -meta.deltaX : meta.deltaX))
          break
        case 'rb':
          corner.right = (corner.right - meta.deltaX) < 0 ? 0 : corner.right - meta.deltaX
          corner.bottom = (corner.bottom - meta.deltaY) < 0 ? 0 : corner.bottom - meta.deltaY
          break
        case 'lb':
          corner.bottom = (corner.bottom - meta.deltaY) < 0 ? 0 : corner.bottom - meta.deltaY
          corner.left = (corner.left + (this.keepRatio ? -meta.deltaX : meta.deltaX)) < 0 ? 0 : (corner.left + (this.keepRatio ? -meta.deltaX : meta.deltaX))
          break
      }
      this.ctrlWidth = this.containerWidth - (corner.left < 0 ? 0 : corner.left) - (corner.right < 0 ? 0 : corner.right)
      this.ctrlHeight = this.containerHeight - (corner.top < 0 ? 0 : corner.top) - (corner.bottom < 0 ? 0 : corner.bottom)
    },
