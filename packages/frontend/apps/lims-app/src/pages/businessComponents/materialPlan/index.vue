        ...
        <view class="content">
          ...
          >
            <view v-if="tabSub.index === 1" class="material-total">
              ...
              <view class="item">
                <view class="title">{{ t('理论量') }}：</view>
                <view class="value">
                  {{
                    `${Number(totalTheoreticalQuantity) || '-'}${current.currentList?.unitName || '-'}`
                  }}
                </view>
              </view>
              <view class="item no-border-right">
                <view class="title">{{ t('计划量') }}：</view>
                <view class="value">
                  {{
...
<script setup>
  ...
  import { useSubTab, useTable, useColumns, useModel } from './hooks';
