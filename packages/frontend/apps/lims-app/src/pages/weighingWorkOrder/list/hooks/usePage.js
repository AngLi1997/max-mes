import { queryWeighCenterExecuteTicketPage } from '@/api';
...
export const usePage = () => {
  ...
  const getWeighingCenterList = async () => {
    let orderSql = '';
    if (params.sortTaskNo && params.sortSendTime) {
      orderSql = `${params.sortTaskNo},${params.sortSendTime}`;
    }
    else {
      orderSql = params.sortTaskNo || params.sortSendTime || '';
    }
    ...
    delete data.sortTaskNo;
    delete data.sortSendTime;
    const res = await queryWeighCenterExecuteTicketPage(data);
  ...
  const itemClick = (item) => {
    uni.navigateTo({
      url: `/pages/weighingWorkOrder/execution/index?id=${item.id}`,
    });
  };
