...
export const useModeDevice = ({ props }) => {
  ...
  const hasAutoPermission = computed(() => hasPermission('121020007000001'));
  const hasManualPermission = computed(() => hasPermission('121020007000002'));
  ...
  const handleNextStep = async () => {
    ...
    uni.navigateTo({
      url: `/pages/weighingWorkOrder/materialWeighing/index?id=${props.id}&mode=${
        mode.value
  ...
  const getWeighCenterExecuteTaskById = async () => {
    ...
    if (hasAutoPermission.value) {
      mode.value = true;
      const station = res.data.stationIdList;
