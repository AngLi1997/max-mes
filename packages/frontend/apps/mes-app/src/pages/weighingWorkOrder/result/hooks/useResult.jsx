...
export const useResult = ({ props }) => {
  ...
  const infoItems = [
    {
      label: t('称量人'),
      field: ['weighUserName', 'weighUserLoginName'],
      type: 'text',
    },
    {
      label: t('复核人'),
      field: ['signUserName', 'signUserLoginName'],
      type: 'text',
    },
  ];
    ...
    const arr = [
      ...
      },
      {
        ...
        customRender: ({ row }) => {
          return (
            <span>
              {row.deviceCode ? `${row.deviceCode}-${row.deviceName}` : ''}
            </span>
          );
        },
      },
      {
        ...
        customRender: ({ row }) => {
          return (
            <span>
              {row.storageCode ? `${row.storageCode}-${row.storageName}` : ''}
            </span>
          );
        },
      },
          ...
          return [
            {
              ...
              onClick: async () => {
                ...
                if (device) {
                  try {
                    let sceneId = row?.categoryInfoType?.value === 0 ? 121001015 : 121002019;
                    if (currentSegmented.value === 1) {
                      sceneId = row?.categoryInfoType?.value === 0 ? 121001018 : 121002022;
                    }
  ...
  const allSigned = computed(() => {
    const { oddmentRecordVOList = [], recordVOList = [] } = resultDetail.value;
    const list = [...oddmentRecordVOList, ...recordVOList];
    return list.every(item => item.signStatus.value === 1) || list.length === 0;
  });
  const leftClick = () => {
    uni.navigateBack();
  };
  ...
  const handleReplace = () => {
    if (resultDetail.value.weighUserId && resultDetail.value.signUserId) {
      if (allSigned.value) {
        replaceValue.value = {
          userName1: '',
          loginName1: '',
  ...
  const segmentedChange = () => {
    const { oddmentRecordVOList, recordVOList } = resultDetail.value;
    tableProps.data = currentSegmented.value ? oddmentRecordVOList : recordVOList;
  };
  // 获取称量结果详情
  const getResultDetail = async () => {
    const res = await queryWeighCenterExecuteTicketWeighRecords(props.id);
  ...
  const signConfirm = async () => {
    try {
      await weighCenterExecuteRequirementSign({
        ticketId: props.id,
        remark: signValue.value.remark,
        signUser: signValue.value.userId2,
      });
  ...
  const replaceConfirm = async () => {
    try {
      await weighCenterExecuteTicketBindOperator({
        ticketId: props.id,
        userId: replaceValue.value.userId1,
        signUser: replaceValue.value.userId2,
      });
  ...
  const getReplaceList = async () => {
    weighCenterExecuteGetSingerListWithPermissionCodeAndStationIds({
      stationIds: resultDetail.value.stationIdList,
    }).then((res) => {
